class Calculator{
    ...
    getDisplayNumber (number) {
        ...
        const intigerDigit = parseFloat(stringNumber.split('.')[0])
        const decimalDigit = stringNumber.split('.')[1]
        let intigerDisplay
        if(isNaN(intigerDigit)){
            intigerDisplay = ''
        } else {
            intigerDisplay = intigerDigit.toLocaleString('en', {maximumFractionDigits: 0})
        }
    ...
    updateDisplay(){
        ...
        if(this.operation != null){
            this.previusOperandTextElement.innerText = `${this.getDisplayNumber(this.previousOperand)} ${this.operation}`
        } else{
